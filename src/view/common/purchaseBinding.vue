...
import { getPurchaseList } from '@/api/user';
export default {
  name: 'purchaseBinding',
  components: {
    dialogCont,
    headerTitle,
    searchForm,
    ...
    return {
      ...
      tableCols: [
        ...
        },
        {
          ...
          width: 150,
          render: (h, scope) => {
            const useItem = this.common().taxRateCode.filter((status) => status.id === scope.row.taxRate[0]);
  ...
  methods: {
    ...
    getDataAllList() {
      ...
      getPurchaseList(parames).then((res) => {
        if (res.code === '200') {
