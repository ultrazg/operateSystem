<template>
  <div id="base">
    ...
    <!-- 核销暂估应付 -->
    <wrapper :key="'writeOff'" :title="'核销暂估应付'" :isFold="false" v-if="form.sourceFrom === '2'">
      <page-table
        ref="writeOffTable"
        col-align="left"
...
export default {
  ...
  methods: {
    ...
    submitForm(val) {
      ...
      const obj = {
        contractId: row.contractId, // 合作合同id
        id: row.id, // 带
        status: row.status, // 带
        ...
        invoiceActualEstimateDetailCommands: row.sourceFrom === '2' ? invoiceActualEstimateDetailCommands : [], // 核销暂估应付
      };

      paymentInfoUpdate(obj).then((res) => {
        if (res.code === '200') {
          this.$notify.success({
            title: '成功',
            message: '修改成功',
    ...
    getDataAllList() {
      ...
      billNumberListForUpdate(params).then((res) => {
        if (res.code === '200') {
    ...
    getWriteOffDataAllList() {
      ...
      invoicePayDetailListForUpdate(params).then((res) => {
        if (res.code === '200') {
          const { data, total } = res.data;
