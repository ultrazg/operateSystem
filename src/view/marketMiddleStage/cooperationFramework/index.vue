          ...
          <template slot-scope="scope" slot="operator">
            <flex>
              ...
              <el-button
                ...
                @click.prevent.stop="
                  openDialog('fabu', {
                    row: { ...scope.row, businessCode: 't_cooperation_framework' },
                    type: 'fabu',
                    configType: 'fabu',
                  })
                "
...
import { getCooperationFrameworkList, cooperationFrameworkDel } from '@/api/marketMiddleStage/cooperationFramework';
...
export default {
  ...
  methods: {
    ...
    handleDelProjectApproval(id) {
      cooperationFrameworkDel(id).then((res) => {
        if (res && res.code === '200') {
          this.$notify({
            title: '成功',
            message: '更新成功',
    ...
    handleDownLoad() {
      this.$confirm('确定下载合作框架模板信息?', '信息', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
      }).then(() => {
        const api = excelDownload;

        api('cooperationFramework').then((res) => {
          const fileName = '合作框架模板信息.xlsx';
