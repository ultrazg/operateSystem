      ...
      tableCols: [
        {
          ...
          render: (h, scope) => {
            ...
            if (d.splitInfoSubDto && d.splitInfoSubDto.length && scope.row.unitName && !d.editRow) {
            ...
            return <div style="display: inline-block;text-overflow: clip;">{html}</div>;
          },
        },
        {
          label: '合作模式',
                ...
                <div>
                  {
                    <el-select
                      ...
                      value={scope.row[scope.column.property] || ''}
                      onChange={(val) => {
                        this.changeSelect(val, scope.row.index, scope.column.property);
                      }}
                    >
                    ...
                    <div>
                      {
                        <el-input
                          ...
                          value={scope.row[scope.column.property] || ''}
                          onInput={(val) => {
                            this.changeInput(val, scope.row.index, scope.column.property);
                          }}
                        ></el-input>
                      }
                    </div>
  ...
  methods: {
    ...
    getDataAllList() {
      ...
      getSplitList(parames).then((res) => {
        if (res.code === '200') {
          ...
          this.tableData = data.map((dataItem, dataIndex) => ({
            ...dataItem,
            index: dataIndex,
    ...
    addRow() {
      ...
      const list = {
        ...
        id: '',
        isAll: '1',
        splitAmount: this.form.balanceAmount,
        splitInfoSubDto: [],
        unitDetailId: '',
        unitName: '',
        defCheckTr: '',
    ...
    tableAction(type, row) {
      ...
      // 新增状态
      if (type === 'save') {
        this.handleUpdateSplit(row.data.index);
      }
      // 删除状态
      if (type === 'del') {
        if (!row.data.id) {
          this.tableData.splice(row.data.index, 1);
          this.editRow = false;
      ...
      // 单个合同状态
      if (type === 'yewu') {
        const parentItem = this.tableData.filter((tableItem) => tableItem.id === row.data.parentId);

        console.log('parentItem', parentItem);
        this.cooperationShow = true;
        this.cooperationData = {
          erpNumber: parentItem && parentItem[0].erpId,
          multipleSelection: [
            {
              ...row.data,
            },
    ...
    handleUpdateSplit(index) {
      ...
      const balanceAmountRevenue = parseFloat(this.form.balanceAmount); // 承揽合同金额
      ...
      // 20220728 去掉校验--张杰
      // if (tableSplitAmount > balanceAmountRevenue) {
      ...
      if (isAll === '1') {
        if (parseFloat(splitAmount) !== balanceAmountRevenue) {
          this.$notifyError('全额分配必须等于项目剩余可分配金额');
