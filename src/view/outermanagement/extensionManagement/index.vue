          ...
          <template slot-scope="scope" slot="operator">
            <flex>
              ...
              <el-button
                ...
                @click.prevent.stop="
                  openDialog('fabu', {
                    row: { ...scope.row, businessCode: 't_finance_delay' },
                    type: 'fabu',
                    configType: 'fabu',
                  })
                "
...
import {
  getExtensionListByConditions,
  getExcel,
  editExtension,
  deleteExtension,
} from '@/api/outermanagement/extensionManagement';
export default {
  ...
  methods: {
    ...
    getDataAllList() {
      ...
      getExtensionListByConditions(parames).then((res) => {
        if (res.code === '200') {
    ...
    openDialog(state, info = {}) {
      ...
      if (state === 'delete') {
        this.$confirm('确定删除当前延期数据?', '信息', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
        }).then(() => {
          const { id } = info.row;

          deleteExtension(row.id).then((res) => {
            if (res && res.code === '200') {
              this.$notify({
                title: '成功',
                message: '删除延期申请成功',
