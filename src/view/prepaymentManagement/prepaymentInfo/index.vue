          ...
          <template slot-scope="scope" slot="operator">
            <flex>
              ...
              <el-button
                ...
                size="small"
                title="发起申请"
                v-if="scope.row.auditStatus === 'verify_draft'"
                icon="iconfont icon-chenggong"
                @click.prevent.stop="
                  openDialog('fabu', {
                    row: { ...scope.row, businessCode: 't_advance_tax' },
                    type: 'fabu',
                    configType: 'fabu',
                  })
                "
...
import { page, exportList, deletePrepaidTax } from '@/api/prepaymentManagement/prepaymentInfo';
export default {
  name: 'prepaymentInfo',
  inject: ['common'],
  data() {
  ...
  methods: {
    ...
    openDialog(state, info = {}) {
      if (state === 'delete') {
        this.$confirm('确定删除当前预缴税数据?', '信息', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
        }).then(() => {
          const { id } = info.row;

          deletePrepaidTax(id).then((res) => {
            if (res && res.code === '200') {
              this.$notify({
                title: '成功',
                message: '删除预缴税数据成功',
    ...
    // 导出
    handleExport() {
      this.$confirm('确定导出预缴税列表信息?', '信息', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
      }).then(() => {
        exportList(this.searchVal).then((res) => {
          const fileName = '预缴税列表信息.xlsx';
